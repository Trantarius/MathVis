...
void DefsPanel::init(){
	...
	header.add_button = GTK_BUTTON(gtk_button_new_from_icon_name("list-add-symbolic"));
	widget_set_margin(GTK_WIDGET(header.add_button),4);
	g_signal_connect(header.add_button,"clicked",G_CALLBACK(DefsPanel::_on_add_button_press),NULL);
	...
	gtk_scrolled_window_set_policy(defs.scroller, GTK_POLICY_NEVER, GTK_POLICY_AUTOMATIC);
...
void DefsEntry::init(){
	...
	widget_set_align(GTK_WIDGET(textedit.error),GTK_ALIGN_START,GTK_ALIGN_END);
	...
	options.remove_button = GTK_BUTTON(gtk_button_new_from_icon_name("edit-delete-symbolic"));
	...
	options.menu_button = GTK_BUTTON(gtk_button_new_from_icon_name("open-menu-symbolic"));
	...
	options.color_button = GTK_COLOR_DIALOG_BUTTON(gtk_color_dialog_button_new(main_ui.color_dialog));
